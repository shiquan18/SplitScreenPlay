...
public class SerialHandle implements Runnable {
    ...
    private static final String TAG = "SerialHandle";
    ...
    public boolean open(String devicePath, int baudrate, int dataBits, int stopBits, int parity, boolean isRead) {
        ...
        try {
            ...
            close();
            isSucc = false;
            serialInter.connectMsg(devicePath , false);
        } finally {
...
//            int size = mBuffInputStream.read(received);//读取以下串口是否有新的数据
            ...
            int len = 0;
            while ((len = mBuffInputStream.read(received)) != -1) {
                //将读取的字节转为字符串对象
                String result = new String(received, 0, len);
                if (serialInter != null) serialInter.readData(path, result, len);
            }
...
//            if (size > 0 && serialInter != null) serialInter.readData(path, received, size);
    ...
    public void send(final String msg) {
        Log.e(TAG,"指令：" + msg);
        try {
            mOutputStream.write(msg.getBytes());//通过输出流写入数据
        } catch (Exception e) {
            e.printStackTrace();
            Log.e(TAG,"串口发送指令失败" + e.toString());
        }
    }
